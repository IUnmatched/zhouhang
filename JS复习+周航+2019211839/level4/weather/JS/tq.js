// 跳转到搜索
var turn = document.querySelector('.newye');
    ...
    $.ajax({
        ...
        success: (e) => {
            ...
            for (var i = 0; i < e.location.length; i++) {
                ...
                a.innerHTML = e.location[i]['name'];
                li.appendChild(a);
                tableul.appendChild(li);
            }

            for (let j = 0; j < e.location.length; j++) {
                tableul.children[j].addEventListener('click', () => {
                    Location = e.location[j]['id'];
                    // 添加记录
                    var lli = document.createElement('li');
                    var aaaa = document.createElement('a');
                    aaaa.innerHTML = e.location[j]['name'];
                    lli.appendChild(aaaa);
                    jilu.appendChild(lli);


                    var cq = document.querySelector('.cq');
                    cq.innerHTML = e.location[j]['name']
                    // 温度下面的小文字
